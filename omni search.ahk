omni_search(start=""){
	...
	while,mm:=list.item[A_Index-1]{
		...
		if IsFunc(mm.text)
			menulist.menu[mm.text]:={launch:"func",name:clean,text:mm.text,type:"menu",sort:mm.text,additional:"Menu"}
		if IsLabel(mm.text)
			menulist.menu[mm.text]:={launch:"label",name:clean,text:mm.text,type:"menu",sort:mm.text,additional:"Menu"}
	}
	...
	for q,b in ffff{
		for a,r in b{
			rating:=0
			for c,d in search{
				RegExReplace(r.sort,"i)" c,"",count)
				if (count<d)
					continue 2
				if count
					rating+=count
			}
			if InStr(r.name,find)
				rating+=10
			if RegExMatch(r.name,"^" find)
				rating+=200
