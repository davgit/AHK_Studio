...
loop,*.ahk
{
	...
	pos:=1
	while,pos:=regexmatch(akmf,"m)^\s*((\w|[^\x00-\x7F])+)\((.+)?\)(\s+)?{",found,pos){
...
loop,*.ahk
{
	...
	while,pos:=regexmatch(akmf,"m)^\s*((\w|[^\x00-\x7F])+)\(.*\)(\s+`;.*(?=\R))*\s*{",found,pos){
		word:=""
		loop,parse,found
		{
			if (a_loopfield!="(")
...
;for a,b in StrSplit(lll," ")
;	list:=RegExReplace(list,"\b" b "\b")
